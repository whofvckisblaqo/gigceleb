...
export default function EditCelebrityForm({ params }) {
  ...
  const handleBookingTypeChange = (key, field, value) => {
    setForm((prev) => ({
      ...
      bookingTypes: {
        ...prev.bookingTypes,
        [key]: { ...prev.bookingTypes[key], [field]: field === "available" ? value : Number(value) },
      },
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to update celebrity"); setSaving(false); return; }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white">Edit Celebrity</h1>
          <p className="text-gray-500 mt-1 text-sm">Updating: <span className="font-semibold text-white">{form.name}</span></p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div key={type.key} className={`border rounded-xl p-4 transition ${form.bookingTypes[type.key].available ? "border-yellow-400/50 bg-yellow-400/5" : "border-zinc-700 bg-zinc-800/50"}`}>
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex items-center gap-3 flex-1">
                    <input type="checkbox" id={`edit-${type.key}`} checked={form.bookingTypes[type.key].available} onChange={(e) => handleBookingTypeChange(type.key, "available", e.target.checked)} className="w-4 h-4 cursor-pointer accent-yellow-400" />
                    <label htmlFor={`edit-${type.key}`} className="text-sm font-semibold text-white cursor-pointer">{type.icon} {type.label}</label>
                  </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button type="submit" disabled={saving} className="w-full sm:w-auto bg-yellow-400 text-black px-10 py-4 rounded-full text-sm font-black hover:bg-yellow-300 transition disabled:opacity-50">
